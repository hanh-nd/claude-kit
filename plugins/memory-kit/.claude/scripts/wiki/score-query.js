...
function countTokenOverlap(queryTerms, docTokens) {
...
function countUsefulAliasOverlap(queryTerms, aliasTokens) {
    ...
    if (usefulMatches.length > 0)
        return usefulMatches.length;
    return matched.length >= 2 ? matched.length : 0;
}
function computeConceptIntentBoost(query, page) {
    if (page.category !== 'concepts' || query.freeTextTokens.length === 0)
        return 0;
    const conceptTokens = tokenize([page.title, page.slug, ...page.aliases, ...page.keyDecisions, page.bodyText].join(' '), NO_STOPWORDS);
...
function hasConceptPathBodySignal(query, page, bodyBM25) {
...
function scorePageImpl(query, page, idf, avgBodyLength, avgSlugLen, avgHeadingLen, avgAliasLen, avgKdLen) {
    ...
    const strongSignal = anchorExactPath > 0 || anchorExactSymbol > 0 || aliasMatch > 0 || aliasBM25 > 0 ||
        filenameBM25 > 0 || headingBM25 > 0 || conceptContentSignal || conceptPathBodySignal;
    const score = anchorExactPath + anchorExactSymbol + aliasMatch + aliasBM25 + filenameBM25 + headingBM25 +
...
export function scoreQuery(query, pages, idf, avgBodyLength, avgSlugLen, avgHeadingLen, avgKdLen, avgAliasLen) {
    if (pages.length === 0)
        return [];
    ...
    const resolvedHeadingLen = avgHeadingLen ?? computeAvgLength(pages.map((p) => tokenize([p.title, ...p.anchors, ...p.aliases].join(' '), NO_STOPWORDS).length));
    const resolvedAliasLen = avgAliasLen ?? computeAvgLength(pages.map((p) => tokenize(p.aliases.join(' '), NO_STOPWORDS).length));
