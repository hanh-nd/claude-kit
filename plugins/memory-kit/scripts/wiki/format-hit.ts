...
function buildFullHit(slug: string, page: WikiPage, displayPath: string, footer: string): string {
  ...
  function assembleLines() {
    ...
    if (keyDecisionBullets.length > 0) {
      ...
      for (const bullet of keyDecisionBullets) lines.push(`- ${bullet}`);
    }
    if (edgeCaseBullets.length > 0) {
      lines.push('');
      lines.push('Edge Cases:');
      for (const bullet of edgeCaseBullets) lines.push(`- ${bullet}`);
    }
    lines.push('');
  ...
  if (snippet.length <= MAX_CHARS) return snippet;
