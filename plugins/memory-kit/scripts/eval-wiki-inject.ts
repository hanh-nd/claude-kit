...
interface EvalExpected {
  expectedSlug?: string;
  expectedReject?: 'gate' | 'strong-signal' | 'threshold' | 'margin' | 'cooldown' | 'session' | 'no-match';
}

function readEvents(eventsPath: string): EvalEvent[] {
...
function readExpected(expectedPath: string): Record<string, EvalExpected> {
  return JSON.parse(fs.readFileSync(expectedPath, 'utf8')) as Record<string, EvalExpected>;
}
