...
function checkPath(p, pol, tool) {
    ...
    if (seg)
        enforce(`Access to sensitive directory '${seg}' is FORBIDDEN.`, pol);
}
function inspectPrompt(prompt, pol) {
    if (typeof prompt !== 'string')
        return;
    const re = /@([^\s]+)/g;
    let match;
    while ((match = re.exec(prompt)) !== null)
        checkPath(match[1], pol, 'UserPromptSubmit');
}
function inspectTool(input, pol) {
